from app.error_handlers.errors import Error, InvalidAttribute
...
class Validator:
    ...
    def validate_schema(self, schema: BaseSchema.__class__, partial=None):
        ...
        if self._request.method != 'POST' and 'uuid' not in self._json['data']:
            raise APIBadRequest(Error('Required UUID member is missing.'))
        if errors:
    ...
    def validate_uuid(self, url_param: str = 'uuid'):
        if self._request.view_args[url_param] != str(self._json['data']['uuid']):
            raise APIBadRequest(InvalidAttribute('uuid in url and uuid in json mismatch.'))
